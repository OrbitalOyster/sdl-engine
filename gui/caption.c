...
SDL_Texture *createCaptionTexture(SDL_Renderer *renderer, TTF_Font *font,
  ...
  // SDL_Surface *tmpSurface = TTF_RenderUTF8_Blended_Wrapped(font, text,
  // *color, 0);
...
SDL_Texture *createOutlinedCaptionTexture(SDL_Renderer *renderer, char *text,
  ...
  SDL_Texture *bg =
      createCaptionTexture(renderer, font->outline, text, outlineColor);
  // Actual text on foreground
  SDL_Texture *fg = createCaptionTexture(renderer, font->ttf, text, color);
  // Get bg dimensions
  int bw;
  int bh;
  ...
  SDL_Rect fgRect = {font->outlineSize, font->outlineSize, fw, fh};
...
void updateCaptionTexture(SDL_Renderer *renderer, Caption *caption) {
  ...
  do {
    ...
    if (c == '\n' || c == '\0') {
      line[ln] = '\0';
      caption->lines[caption->numberOfLines] = calloc(ln + 1U, sizeof(char));
      // snprintf(caption->lines[caption->numberOfLines], MAX_CAPTION_LINE_SIZE,
      // "%s", line);
      strcpy(caption->lines[caption->numberOfLines], line);
      caption->textures[caption->numberOfLines] = createOutlinedCaptionTexture(
          renderer, line, caption->font, caption->color, caption->outlineColor);
  ...
  // INFOF("Updating caption texture. Number of lines: %u",
  // caption->numberOfLines);
}

Caption *createCaption(SDL_Renderer *renderer, int32_t x, int32_t y, char *text,
                       Font *font, SDL_Color *color, SDL_Color *outlineColor) {
